...
sys.path.insert(2, os.path.join(sys.path[0], '../bias_probing'))
...
from transformers import set_seed, is_wandb_available, BertTokenizerFast, HfArgumentParser, BertConfig, BertModel, \
    WEIGHTS_NAME, BertForSequenceClassification, RobertaTokenizerFast, RobertaConfig, RobertaModel, RobertaForSequenceClassification
...
def build_probe(input_size, num_classes=2, probe_type='mlp', augment_z=False):
    if augment_z:
        input_size += 1
    probes = {
...
def build_encoder(model_name_or_path, return_tokenizer=False, base_dir=None):
    ...
    if model_name_or_path == 'bert-random':
        ...
        config = RobertaConfig.from_pretrained(real_path)
        if config.architectures and 'BertWithWeakLearner' in config.architectures:
            ...
            #encoder = bert_with_weak.bert
            encoder = bert_with_weak.roberta
        else:
            print("XXXXX In here XXXX")
            #model = BertForSequenceClassification.from_pretrained(real_path)
            #model = AutoModelForSequenceClassification.from_pretrained(real_path)
            #encoder = model.bert
            model = RobertaForSequenceClassification.from_pretrained(real_path)
...
# used to just the predict the NLI label
class NLIClassMapper:
    def __init__(self):
        print("NLI Class Mapper")
    
    def __call__(self, examples):
        examples['probing_label'] = examples['label']
        return examples
    
class BiasMapper:
    def __init__(self):
        print("Synthetic Bias Mapper")
        
    def __call__(self, examples):
        examples['probing_label'] = examples['bias']
        return examples       
...
def get_mapper(config: TaskConfig, dataset_info: DatasetInfo):
    ...
    elif config.type == 'nli-label':
        return NLIClassMapper()
...
def prepare_dataset(args: ExperimentArguments, config: TaskConfig, _embed_and_cache_flag=True):
    ...
    if args.augment_z or args.save_groups:
        def collate(samples):
            inputs = {
            ...
            nuisance = torch.tensor([sample['wo_overlap'] for sample in samples])
            return (
        ...
        def collate(samples):
            ...
            probing_labels = torch.tensor([sample['probing_label'] for sample in samples])
            nli_labels = torch.tensor([sample['label'] for sample in samples])
    ...
    train_dataset = embed_and_cache(task_name, train_dataset, 'train', encoder, real_path, collate_fn=collate,
                                    num_labels=num_labels, batch_size=batch_size, device=device, overwrite_cache=overwrite_cache,
                                    augment_z=args.augment_z, save_groups=args.save_groups, eval_groups=args.eval_groups)
...
def run_online_coding_experiment(args: ExperimentArguments, config: TaskConfig, device=None):
    ...
    def create_probe():
        if config.type == "nli-label":
            ...
            return build_probe(encoding_size, num_classes=3, probe_type=probe_type, augment_z=augment_z)
        else:
            print("Number of classes = 2")
            return build_probe(encoding_size, probe_type=probe_type, augment_z=augment_z)
    ...
    uniform_cdl, online_cdl = online_code_probe.evaluate(train_dataset, test_dataset, val_dataset,
                                                         reporting_root=reporting_root, verbose=True, device=device, eval_groups=args.eval_groups)
...
def task_config(args: ExperimentArguments) -> TaskConfig:
    ...
    config_file_path = join(config_dir, task_config_file)
