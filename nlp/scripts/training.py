...
from transformers.models.roberta import RobertaTokenizerFast, RobertaForSequenceClassification
...
def paired_accuracy(labels, preds):
    
    acc = ((labels[::2] == preds[::2]) & (labels[1::2] == preds[1::2])).mean()
    return acc   
    

def compute_metrics_paired(pred):
    labels = pred.label_ids
    preds = pred.predictions.argmax(-1)
    acc = accuracy_score(labels, preds)
    paired_acc = paired_accuracy(labels, preds)
    return {
        'accuracy': acc,
...
def _select_model_configuration(cfg: DictConfig) -> Tuple[PreTrainedModel, BertConfig, Type[Trainer]]:
    ...
    if run_type == 'simple' or run_type == 'hypo_only':
        ...
        # Train a model so that z can't be predicted from r(x)
        
        config = BertAdversarialConfig.from_pretrained(model_name_or_path,
                                                       num_labels=3,
                                                       num_z_labels=2,
                                                       lambda_reversal=0.5)
        model = BertAdversarial.from_pretrained(model_name_or_path, config=config)
        ...
        if loss_fn.startswith('cr/'):
            ...
            if dataset_name == 'multi_nli' or dataset_name == "multi_nli_balanced":
            ...
            config = BertWithExplicitBiasConfig.from_pretrained(model_name_or_path,
                                                                ...
                                                                loss_fn=loss_fn,
                                                                weak_model_name_or_path="/scratch/nhj4247/robustness/bias-probing/models/seed:42/synthetic_weak_dummy/train.csv")
            model = BertWithExplicitBias.from_pretrained(model_name_or_path, config=config)
            #model = BertWithExplicitBias(config=config)
...
def _select_preprocess_func(cfg: DictConfig, is_train=True, premise_key='premise', hypothesis_key='hypothesis'):
    ...
    if (not run_type == "adversarial") and (((is_train and loss_fn.startswith('explicit/'))
    ...
    if is_train and run_type == "adversarial":
        def preprocess_function(examples):
            ...
            if 'wo_overlap' in examples:
                result = {
                    **result,
                    'z_labels': examples['wo_overlap'],
                }
            return result
        return preprocess_function
...
def main(cfg: DictConfig):
    ...
    training_args = TrainingArguments(
        ...
        remove_unused_columns=isinstance(model, RobertaForSequenceClassification),
        skip_memory_metrics=True,
        group_by_length=True,
        seed=seed,
        report_to=['wandb'] if report_to_wandb and is_wandb_available() else []
    )
        ...
        for eval_ds_name, eval_ds, info in test_datasets:
            ...
            if info.binerize:
                ...
                def compute_metrics_with_name(pred):
                    results = compute_metrics_paired(pred)
                    return {
            ...
            eval_output = trainer.evaluate(eval_dataset=eval_ds)
            print(eval_output)
            with open(os.path.join(output_dir, eval_ds_name + '_results.txt'), 'w') as f:
