...
from transformers.models.roberta import RobertaConfig, RobertaForSequenceClassification, RobertaModel #, RobertaPreTrainedModel
...
class BertWithWeakLearnerConfig(RobertaConfig):
...
class BertWithWeakLearner(RobertaModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None, id=None, **kwargs):
        ...
        weak_logits = self._forward_weak(inputs_with_labels, id)
        # TODO Remove
...
class BertWithExplicitBias(RobertaPreTrainedModel):
    ...
    def _encode_hypothesis_only(self, hypothesis_input_ids, hypothesis_attention_mask):
        with torch.no_grad():
            _, bias_pooled_output = self.roberta(hypothesis_input_ids, token_type_ids=None,
    ...
    def _forward_hans_features(self, hans_features: Iterable[torch.Tensor],
        ...
        if premise_input_ids is not None and hypothesis_input_ids is not None and len(self.similarity_types) > 0:
            with torch.no_grad():
                h_outputs = self.roberta(hypothesis_input_ids, token_type_ids=None,
                                      attention_mask=hypothesis_attention_mask, return_dict=False)
                h_matrix = self.dropout(h_outputs[0])
                # h_matrix = h_outputs[0]

                p_outputs = self.roberta(premise_input_ids, token_type_ids=None, attention_mask=premise_attention_mask,
                                      return_dict=False)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        # Main model
        _, pooled_output = self.roberta(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,
                                     return_dict=False)
